...
if($act=='insert')
{
    ...
    $cek = mysql_fetch_assoc($cekquery);

    if ($cek['enrol_key'] == $_POST['enrol']) {
      $cekdaftar = mysql_query("SELECT count(exm_schedule_id) AS jml FROM exm_reg WHERE exm_schedule_id = '$_POST[id]' ");
      $cd = mysql_fetch_assoc($cekdaftar);
      $x0 = $cd['jml'] + 1 ;
